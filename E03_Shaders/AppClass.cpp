...
{
	// create the window
	m_pWindow = new Window(VideoMode(800, 600), m_sWindowName, Style::Default, ContextSettings(32));
...
void AppClass::ProcessKeyboard(Event a_event)
{
	
	if (a_event.key.code == Keyboard::Key::Escape)//Event says I pressed the Escape key
		m_bRunning = false;
	else if (Keyboard::isKeyPressed(Keyboard::Num1)) //I am currently pressing the Num1 (not the same as above)
	...
	if (m_kHoldKey == a_event.key.code)
		m_bComplimentary = !m_bComplimentary;
	m_kHoldKey = a_event.key.code;
}
